...
Invoke-WebRequest -UseBasicParsing -Uri $path_Cinderella -OutFile "$env:TEMP/AUTOKETCINDYINSTALL/Cinderella-3.0b.2080-64bit.exe"
echo "Installing Cinderella..."
.\Cinderella-3.0b.2080-64bit.exe -q

echo "Downloading KeTTeX..."
Invoke-WebRequest -UseBasicParsing -Uri $path_kettex -OutFile "$env:TEMP/AUTOKETCINDYINSTALL/kettex.zip"
...
echo "Downloading R..."
Invoke-WebRequest -UseBasicParsing -Uri $path_R -OutFile "$env:TEMP/AUTOKETCINDYINSTALL/R-4.3.1-win.exe"
echo "Installing R..."
.\R-4.3.1-win.exe /silent

echo "Downloading SumatraPDF..."
Invoke-WebRequest -UseBasicParsing -Uri $path_sumatra -OutFile "$env:TEMP/AUTOKETCINDYINSTALL/SumatraPDF-3.4.6-64-install.exe"
echo "Installing SumatraPDF..."
.\SumatraPDF-3.4.6-64-install.exe -s -d "C:\Program Files\SumatraPDF"

echo "Downloading Maxima..."
Invoke-WebRequest -UseBasicParsing -Uri $path_maxima -OutFile "$env:TEMP/AUTOKETCINDYINSTALL/maxima-5.47.0-win64.exe"
